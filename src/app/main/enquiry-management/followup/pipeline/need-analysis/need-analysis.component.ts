...
export class NeedAnalysisComponent implements OnInit {
    ...
    ngOnInit(): any {
        ...
        this.enquiryService.getAllNeedAnalysisEnquiriesByCampus(this.campusId).subscribe((enqRes: Enquiry[]) => {
            if (enqRes.length === 0) {
                this.matSnackBar.open('No Need Analysis Enquiry Found', null, {
                    duration: 3000
                });
            }
