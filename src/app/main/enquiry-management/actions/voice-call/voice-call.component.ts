...
export class VoiceCallComponent implements OnInit {
    ...
    constructor(
        ...
        private dialogRef: MatDialogRef<VoiceCallComponent>,
    ...
    onAvailavle(e): void {
        ...
        const probability = this.form.get('probability');
        if (e.source.value === false) {

            this.isAvailavle = true;
            status.clearValidators();
            status.updateValueAndValidity();
            probability.clearValidators();
            probability.updateValueAndValidity();

        } else if (e.source.value === true) {

            this.isAvailavle = false;
            status.setValidators(Validators.required);
            status.updateValueAndValidity();
            probability.setValidators(Validators.required);
            probability.updateValueAndValidity();
        }
    }
    ...
    onVoiceCall(): void {
        ...
        if (
            ...
            if (this.isAvailavle === false) {
                ...
                this.enquiryService.voiceCall(voiceCallData).subscribe((vcRes: any) => {
                    // console.log(vcRes);
                    this.form.reset();
                    this.dialogRef.close({close: 'closeAfterSaved'});
                    this.matSnackBar.open('Voice Call Saved Successfully', null, {
            ...
            } else if (this.isAvailavle === true) {

                const voiceCallData = {
                    type: 'voice_call',
                    status: this.enquiry.enquiry.status,
                    inquiryId: this.enquiry.enquiry.id,
                    userId: this.user.id,
                    next_follow_up_date: data.next_follow_up_date._i.year
                        + '-' +
